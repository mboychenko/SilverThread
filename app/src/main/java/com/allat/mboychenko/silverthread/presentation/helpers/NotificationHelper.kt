...
import com.allat.mboychenko.silverthread.presentation.receivers.TimerNotificationActionReceiver
...
private const val CHANNEL_NAME_TIMER = "Allat Notifications"
...
fun showNotification(context: Context, notificationCode: AlarmNotificationCodes, extras: Bundle?) {
    ...
    val nBuilder = getBasicNotificationBuilder(context, CHANNEL_ID_TIMER, playSound)
    ...
    if (notificationCode == AlarmNotificationCodes.START || notificationCode == AlarmNotificationCodes.END) {
    ...
    if (notificationCode == AlarmNotificationCodes.BEFORE || notificationCode == AlarmNotificationCodes.BEFORE_UPDATE) {
        val oneMin = TimeUnit.MINUTES.toMillis(1)
        if (remindBeforeUpdate - oneMin > TimeUnit.MINUTES.toMillis(5)) {
            setAlarm(context, TimeUnit.MINUTES.toMillis(1),
    ...
    nManager.createNotificationChannel(context, CHANNEL_ID_TIMER, CHANNEL_NAME_TIMER, playSound)
...
private fun NotificationManager.createNotificationChannel(
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        ...
        nChannel.setSound(Uri.parse("android.resource://" + context.packageName + "/" + R.raw.ring), audioAttributes)
